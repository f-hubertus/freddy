...
use Getopt::Long;
use File::Copy;
# The next uses are added for the Galaxy enviroment
use Archive::Tar;
use Archive::Extract;
use Cwd;
...
# this script.
my $FFPred_dir = '/home/floris/galaxy-dist/tools/function_prediction/FFPred2';
...
# Sample usage :
# 
# ./gNbFFPred.pl -i new_test_out/d5uhj12/d5uhjvhfuscoharf.fsa -i new_test_out/d5uhj12/d5uhjvhfuscoharf.masked3.chk -i new_test_out/d5uhj12/d5uhjvhfuscoharf.unmasked2.chk -i new_test_out/d5uhj12/d5uhjvhfuscoharf.unmasked3.chk -o new_test_out/d5uhj12/

### -o is optional, the default output folder is FFPred2/out

# Overview of command sample usage
#./gNbFFPred.pl -i new_test_out/d5uhj12/d5uhjvhfuscoharf.fsa 
#	-i new_test_out/d5uhj12/d5uhjvhfuscoharf.masked3.chk 
#	-i new_test_out/d5uhj12/d5uhjvhfuscoharf.unmasked2.chk 
#	-i new_test_out/d5uhj12/d5uhjvhfuscoharf.unmasked3.chk 
#	-o new_test_out/d5uhj12/
...
my ($arrayjob, @input_array, $dirOut); # Substituted $fasta with @input_array
my $args = GetOptions(
                       "a"   => \$arrayjob,
		       "i=s" => \@input_array, # Substituted $fasta with @input_array
                       "o=s" => \$dirOut
                     );

# input 1, fasta file
my $fasta = $input_array[0]; # Declare the fasta file from @input_array
...
foreach my $id (keys %$inputs)
{
    ...
    MkDir($jobs_directory, $job, @input_array); # Substituted $fasta with @input_array (which includes the fasta file)
...
sub MkDir # This is the generic output dir in the given output dir
{
    my ($jobs_dir, $job_hash, @input_array)= @_; # Substituted $fasta with @input_array
...
########################################################################################################################
# This part is for the Galaxy enviroment, Galaxy cannot read .chk files and wont upload them properly
# Compress them into .tar.gz (yes both!, Galaxy unzips them automaticly so .tar remains), the .tar files are copied to the 'job' dir
# The copied .tar files are put into an array, then changing to the job dir
# In the job dir all the files in that array get extracted, finally changing back to original working dir to prevent bugs

	copy("$input_array[0]","$job_hash->{'dir'}/$job_hash->{'md5'}.fsa") or die "Copy fasta failed: $!";
	copy("$input_array[1]","$job_hash->{'dir'}/$job_hash->{'md5'}.masked3.chk.tar") or die "Copy masked3.chk.tar failed: $!";
	copy("$input_array[2]","$job_hash->{'dir'}/$job_hash->{'md5'}.unmasked2.chk.tar") or die "Copy unmasked2.chk.tar failed: $!";
	copy("$input_array[3]","$job_hash->{'dir'}/$job_hash->{'md5'}.unmasked3.chk.tar") or die "Copy unmasked3.chk.tar failed: $!";

	my $m3_c_t = "$job_hash->{'dir'}/$job_hash->{'md5'}.masked3.chk.tar";
	my $um2_c_t = "$job_hash->{'dir'}/$job_hash->{'md5'}.unmasked2.chk.tar";
	my $um3_c_t = "$job_hash->{'dir'}/$job_hash->{'md5'}.unmasked3.chk.tar";
	my @tar_array = ($m3_c_t, $um2_c_t, $um3_c_t);

	my $start_dir = getcwd;
	chdir "$job_hash->{'dir'}/";	
	foreach my $file(@tar_array){
...
sub RunFeaturama
{
    ...
    my $featurama = "$rootdir/featurama/nbfeat.pl " . # Substituted features.pl with nbfeat.pl (no blast features)
                    "-d $job_hash->{'dir'} " . 
