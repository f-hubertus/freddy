...
my $ncbidir = '/scratch2/floris/function_prediction/FFPred2/bin';
my $dbname = '/scratch2/floris/function_prediction/uniprot/uniprot_cyanobacteria_viridplantae.masked';
...
my %models = ('MEMSAT-SVM_w27_RE.model'=>27,'MEMSAT-SVM_w27_SP.model'=>27,'MEMSAT-SVM_w33_HL.model'=>33,'MEMSAT-SVM_w35_IO.model'=>35,'MEMSAT-SVM_w33_GM.model'=>33);
my @windows = ("27","33","35","GM");

my (@mtx,$blast_out,$svm_all,$memsat_out,$memsat_out_single,$globmem_out,$png_schematic_out,$png_cartoon_out,$png_cartoon_memsat3_out,%range,%raw_hl,%raw_io,%raw_re,%raw_sp,%mtx_list,$header,@constraints);
...
sub main{
	...
	#$system = `rm $input_path/$header* &> /dev/null` if $remove_files; # REMOVED for FFPred 2.0
	...
	# Clean up
	#$system = `rm $input_path/$header* &> /dev/null`; # REMOVED for FFPred 2.0
...
sub get_arguments{
	...
	if (!$ARGV[0]){
		...
		my $result = GetOptions ("n=s" => \$ncbidir,
					...
					"i=s" => \$input_path, # NEW for FFPred 2.0
					"j=s" => \$output_path,
					"w=s" => \$mem_dir,
				 	"e=i" => \$remove_files,
				 	"p=i" => \$globmem,
		...
		#$input_path = $mem_dir.'input/'; # REMOVED due to change above for FFPred 2.0
		...
		if ($system){
			...
			if ($system){
				...
				unless(`perl -MGD -e 1 &> /dev/stdout`){
					...
					&draw_image($tmp[0],$seq,$topology_array,$pred_n, $re_helix_array,$pred_signal) if $graphics;
